...
#define DRIVER_DESC			"Pin control driver for Hilscher netX4000 platforms"
#define DRIVER_NAME			"pinctrl-single-netx4000"
...
static int pcs_add_more_gpio_func(struct device_node *node, struct pcs_device *pcs)
{
	...
	while (1) {
		...
		for (i = 0; ; i++) {
			...
			dev_info(pcs->dev, "range%d: offset %d, npins %d, gpiofunc 0x%x\n", rangeNo, range->offset, range->npins, range->gpiofunc);
			mutex_unlock(&pcs->mutex);
		}
		if (ret)
			break;
...
static int pcs_add_gpio_func(struct device_node *node, struct pcs_device *pcs)
{
	...
	for (i = 0; ; i++) {
		...
		range->gpiofunc = gpiospec.args[2];
		mutex_lock(&pcs->mutex);
		dev_info(pcs->dev, "range: offset %d, npins %d, gpiofunc 0x%x\n", range->offset, range->npins, range->gpiofunc);
		list_add(&range->node, &pcs->gpiofuncs);
		mutex_unlock(&pcs->mutex);
	}
...
// 	{ .compatible = "pinconf-single", .data = &pinconf_single },
	{ .compatible = "pinctrl-single-netx4000", .data = &pinctrl_single },
	{ },
};
